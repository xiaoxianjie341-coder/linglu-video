# Linglu Video — Project Structure Design
...
Linglu Video is Linglu's AI-powered short-form video generation module. The Linglu agent orchestrates the full pipeline — scene planning, asset generation via fal.ai, composition via Remotion, and post-production via FFmpeg. Outputs in both YouTube (16:9) and social media (9:16) formats from a single run.
...
├── README.md
└── SKILL.md                    # Linglu agent behavior rules only
```
...
| `preferences.json` | Per-user creative defaults — generated by `linglu-video setup` | Generated by CLI |
...
│   ├── studio.ts                     # linglu-video studio (launches Remotion)
│   └── setup.ts                      # linglu-video setup (writes preferences.json)
│
...
```
CLI command (linglu-video generate --workflow horror.json)
  |
  v
